...
public class AprioriGen {
	...
	public static void main(String[] args) throws Exception {
		...
		MongoCollection<Document> ck = db.getCollection(mongoColCK);
		 ...
		 Iterable<Document> result = lKMinusOne.aggregate(Arrays.asList(new Document("$unwind",
    		new Document("path", "$items")),
   		 	new Document("$group",
		    new Document("_id", "$items")),
    	    new Document("$project",
			new Document("items", "$_id")),
    	    new Document("$count", "items")));

		for (Document vals : result)
		{

			int k = vals.getInteger("items");
			int kMin1= k - 1;
			System.out.println(kMin1);
...
//		 * 	p.pos_k-1<q.pos_k-1.
		...
		Iterable<Document> itemsArray = lKMinusOne.aggregate(Arrays.asList(new Document("$project",
						new Document("items",
								new Document("$objectToArray", "$items"))),
				new Document("$sort",
						new Document("items", 1L))));

		for (Document doc : itemsArray){
